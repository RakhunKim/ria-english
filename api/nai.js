...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (vibeRef) {
      const isDataUrl = vibeRef.startsWith('data:');
      const mimeType = isDataUrl ? vibeRef.split(';')[0].split(':')[1] : 'image/png';
      const b64 = isDataUrl ? vibeRef.split(',')[1] : vibeRef;
      const imgBuf = Buffer.from(b64, 'base64');
      const ext = mimeType.includes('jpeg') ? 'jpg' : 'png';
      form.append(FORM_PART_NAME, imgBuf, {
