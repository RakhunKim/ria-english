...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // reference_image_multiple — data URL이면 순수 base64만 추출
    if (parsed.parameters?.reference_image_multiple) {
      parsed.parameters.reference_image_multiple =
        parsed.parameters.reference_image_multiple.map(img => {
    ...
    console.log('[NAI] has ref:', !!(parsed.parameters?.reference_image_multiple?.length));
